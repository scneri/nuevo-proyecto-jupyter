...
cat_pos = [500, 40]
player_speed = 5
cat_direction = "down"  # Dirección inicial
#Niveles
nivel=1
activ1=False
activ2=False
...
while inicio:
    ...
    if nivel == 1:
        ...
        cat_rect_scaled.x += 2  # Movimiento hacia la derecha
        if cat_rect_scaled.left > 800:
            cat_rect_scaled.right = 0  # Vuelve al inicio
        ...
        if clone_b.draw():#clona al gato cafe
            nivel=nivel+1
        
        if clone_o.draw():#clona al gato negro
            nivel=nivel+2

        if next_aux.draw(): #siguiente
            nivel = 12  # CAMBIAR DE NIVEL
...
#---NIVEL 2 ---INTRO CHARACTER
        ...
        screen.blit(text2, (100, 150))
        # Escalarlo
        sprite_actual = cat_sprites2[int(frame_count) // 10 % 3] #hace que cada sprite se muestre durante 10 frames antes de pasar al siguiente.
